...
const mockPostTokenVerify = postTokenVerify as jest.MockedFunction<typeof postTokenVerify>;
const mockPostChangePassword = postChangePassword as jest.MockedFunction<typeof postChangePassword>;
...
describe('NewPassword', () => {
  ...
  it('renders the component with correct form elements', () => {
    ...
    expect(screen.getByPlaceholderText('login.chooseNewPassword')).toBeInTheDocument();
    expect(screen.getByPlaceholderText('login.pleaseConfirmNewPassword')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /login.updatePassword/i })).toBeInTheDocument();
  });
  ...
  it('submits form with valid password', async () => {
    ...
    const passwordInput = screen.getByPlaceholderText('login.chooseNewPassword');
    const confirmInput = screen.getByPlaceholderText('login.pleaseConfirmNewPassword');
    const submitButton = screen.getByRole('button', { name: /login.updatePassword/i });
    ...
    await waitFor(() => {
      expect(mockPostChangePassword).toHaveBeenCalledWith('test-token-123', 'newPassword123');
    });
  });
  ...
  it('shows loading state during form submission', async () => {
    mockPostChangePassword.mockImplementation(() => new Promise(resolve => setTimeout(() => resolve(true), 100)));
    ...
    expect(screen.getByTestId('loading-icon') || screen.getByRole('img', { hidden: true })).toBeInTheDocument();
  });
  ...
  it('requires password field', async () => {
    ...
    await waitFor(() => {
      expect(screen.getAllByText('login.pleaseInputNewPassword')[0]).toBeInTheDocument();
    });
  });
});
