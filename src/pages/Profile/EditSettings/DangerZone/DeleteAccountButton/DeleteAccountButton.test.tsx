...
describe("DeleteAccountButton", () => {
  ...
  it("shows confirmation button when delete button is clicked", () => {
    ...
    expect(screen.getByRole("button", { name: /are you sure\? \.\.\. 5/i })).toBeInTheDocument();
  });
  ...
  it("counts down from 5 to 1 and then hides confirmation button", async () => {
    ...
    expect(screen.getByRole("button", { name: /are you sure\? \.\.\. 4/i })).toBeInTheDocument();
    
    // Advance timer by another second
    act(() => {
      jest.advanceTimersByTime(1000);
    });
    expect(screen.getByRole("button", { name: /are you sure\? \.\.\. 3/i })).toBeInTheDocument();
    
    // Advance timer by another second
    act(() => {
      jest.advanceTimersByTime(1000);
    });
    expect(screen.getByRole("button", { name: /are you sure\? \.\.\. 2/i })).toBeInTheDocument();
    
    // Advance timer by another second
    act(() => {
      jest.advanceTimersByTime(1000);
    });
    expect(screen.getByRole("button", { name: /are you sure\? \.\.\. 1/i })).toBeInTheDocument();
    ...
    expect(screen.queryByRole("button", { name: /are you sure/i })).not.toBeInTheDocument();
  ...
  it("calls archiveAccount and logout when confirmation button is clicked", () => {
    ...
    const confirmButton = screen.getByRole("button", { name: /are you sure\? \.\.\. 5/i });
    act(() => {
