  ...
  useTranslation: () => ({
    t: (key: string) => {
      const translations: Record<string, string> = {
        ...
        "profile.settingSendEmailOnFriendRequest":
          "Send email on friend request",
        "profile.settingSendEmailWhenNewMessage": "Send email when new message",
        "profile.settingKeepMeInformedAboutRewaer":
          "Keep me informed about Rewaer",
      };
...
describe("EmailSettings", () => {
  ...
  it("renders the email settings section with correct structure", () => {
    ...
    expect(
      screen.getByText("Send email on friend request"),
    ).toBeInTheDocument();
    expect(screen.getByText("Send email when new message")).toBeInTheDocument();
    expect(
      screen.getByText("Keep me informed about Rewaer"),
    ).toBeInTheDocument();
  });
